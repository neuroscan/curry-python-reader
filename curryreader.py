...
def read(inputfilename='', plotdata = 1, verbosity = 2):
    ...
    if extension == 'dat':
        ...
        raise Exception("Unsupported File Name (choose a .cdt or .dat file)")
    ...
    if not contents:
        try:
            ...
            raise Exception("Could not open alternative parameter file. Without parameter file, data cannot be read")
    
    fid.close()
    
    if not contents:
        raise Exception("Parameter file is empty. Without parameter file, data cannot be read")
    ...
    if ix != -1 :
        ...
        if text == '{2912E8D8-F5C8-4E25-A8E7-A1385967DA09}':
            raise Exception ("Unsupported data format (compressed)", "Use Curry to convert this file to Raw Float format")
